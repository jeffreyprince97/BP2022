...
if __name__ == "__main__":
    ...
    with open(os.path.join(os.getcwd(), "comments.txt"),'w') as f:
        ...
        f.write("%s;%s;%s;%s;%s;%s;\n" %("frame","Ballon","Cirkel","Perle","Skumballon", "Vat"))
    ...
    for i, image_path in enumerate(images_to_classify):
        try:
            ...
            for p in outputs["predictions"]:
                ...
                elif p["label"] == "Skumballon":
                    output_list_Skumballon.append(p["confidence"])
            ...
            if i % 10 == 0:             # ??
                ...
                with open(os.path.join(os.getcwd(), "comments.txt"),'a') as f:
                    for kk, p in enumerate(output_list_Ballon):
                        f.write("%s;%s;%s;%s;%s;%s;\n" %(kk+i,output_list_Ballon[kk],output_list_Cirkel[kk],output_list_Perle[kk],output_list_Skumballon[kk],output_list_Vat[kk]))
                        # f.write("%s;%s;%s;%s;%s;%s;\n" %(kk+i*printbatches,output_list_Ballon[kk],output_list_Cirkel[kk],output_list_Perle[kk],output_list_Ballon[kk],output_list_Vat[kk]))
